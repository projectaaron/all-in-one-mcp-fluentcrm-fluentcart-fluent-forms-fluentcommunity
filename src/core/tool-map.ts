 ...
 *  surface in seconds. No args → one line per area (the ~47 resource
...
import { placeholdersOf } from './tool-factory.js';
...
const isListAction = (name: string) => /^(list|search|get_all)/.test(name);
...
function toolLine(t: MapTool): string {
...
export function renderOverview(areas: MapArea[]): string {
  ...
  const products = [...new Set(areas.map((a) => a.product))]
    .map((p) => `${p}: ${areas.find((a) => a.product === p)!.enabled ? 'enabled' : 'not configured'}`)
    .join(' · ');
  const lines = [
    `fluentMCP tool map — ${toolCount} tools in ${areas.length} areas. ${products}`,
    MAP_CONVENTIONS,
...
export function renderArea(a: MapArea): string {
  ...
  return [head, ...a.tools.map((t) => `  ${toolLine(t)}`)].join('\n');
}
...
export function buildInstructions(areas: MapArea[], mode: ToolMode): string {
  ...
  return [
    `fluentMCP: ${toolCount} tools for the WordPress site's Fluent products (${products}).`,
    naming,
