...
export interface VerifyResult {
  ...
  /** Human-readable per-product lines (✅ / ⏭️ / ❌). */
  lines: string[];
}

/** The checks behind verify_setup, reusable by support_report: one
 *  namespace index read, then one harmless authenticated read per product. */
export async function runVerify(entries: ProductEntry[], config: ServerConfig): Promise<VerifyResult> {
...
export function registerVerifySetup(server: McpServer, entries: ProductEntry[], config: ServerConfig): void {
  server.registerTool(
    ...
    async () => {
      const { ok, reports, lines } = await runVerify(entries, config);
      return {
