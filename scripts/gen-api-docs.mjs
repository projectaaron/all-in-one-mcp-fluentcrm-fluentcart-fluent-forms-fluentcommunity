   ...
   pipeline covers every product (all automatic — no hand-maintained list):
     ...
     3. Write, per product:
          ...
          docs/api-reference/<product>.md           overview: auth + one table per group,
                                                    linking to the vendor's own schema docs
          (Per-endpoint request/response schemas are NOT mirrored here — they are
          WPManageNinja's documentation; the overview links to it instead.)
...
async function generate(productKey) {
  ...
  /* Write `file` unless today's run would change nothing but the datestamp.
     The refresh workflow detects changes with `git status --porcelain`, so a
     stamp that moves on every run raises a no-op PR every week — and weekly
     noise trains reviewers to rubber-stamp the diff, which is how a real
     upstream change once slipped through unnoticed.

     The test is exact rather than a date-shaped regex over the diff: re-render
     with the date already committed, and if that reproduces the file
     byte-for-byte then the stamp was the only thing that would have moved.
     A file that survives this keeps the date its content was actually
     captured on, which is the honest reading — a later scrape returning
     identical bytes did not recapture anything. */
  function writeStamped(file, render, previousDate) {
    if (fs.existsSync(file)) {
      const prev = fs.readFileSync(file, 'utf8');
      const stamped = previousDate(prev);
      if (stamped && render(stamped) === prev) {
        console.log(`  ${file} — unchanged since ${stamped}, keeping its datestamp`);
        return false;
      }
    }
    fs.writeFileSync(file, render(today));
    return true;
  }
  let server = '';
  let grandTotal = 0;
  ...
  writeStamped(
    ...
    (d) => JSON.stringify({ product: productKey, source: product.docsUrl, scraped: d, baseUrl: server, namespace: product.namespace, count: grandTotal, endpoints: inventory }, null, 2) + '\n',
    (prev) => { try { return JSON.parse(prev).scraped ?? null; } catch { return null; } }
  );

  // overview file: one table per group
  const renderOverview = (d) => {
    let ov = `# ${product.title} REST API — Reference\n\n`;
    ov += `> **Source:** <${product.docsUrl}> · **Scraped:** ${d} · **Endpoints:** ${grandTotal} across ${indexRows.length} groups\n`;
    ...
    ov += `Request/response schemas for every endpoint below are in ${product.title}'s own developer documentation: <${product.docsUrl}> (source: <https://github.com/WPManageNinja>). This file is an inventory, not a copy of that documentation.\n\n`;
    ov += `## Groups\n\n| Group | Endpoints |\n|-------|-----------|\n`;
    for (const [t, c] of indexRows) ov += `| ${t} | ${c} |\n`;
    ov += `\n## Endpoints by group\n\n${overviewSections.join('\n')}\n`;
    ov += `_Generated by \`scripts/gen-api-docs.mjs\` from the per-operation OpenAPI specs published at ${product.host.replace('https://', '')}; endpoints marked (Pro) require the product's Pro version._\n`;
    return ov;
  };
  writeStamped(
    product.overviewFile,
    renderOverview,
    (prev) => prev.match(/\*\*Scraped:\*\* (\d{4}-\d{2}-\d{2})/)?.[1] ?? null
  );

  // Per-group schema files are no longer written (they mirrored the vendor's
  // docs); remove any left over from older runs. Only endpoints.json lives here.
  for (const f of fs.readdirSync(product.outDir)) {
    if (f.endsWith('.md')) {
      fs.unlinkSync(`${product.outDir}/${f}`);
      console.log(`[${product.title}] removed legacy group doc:`, f);
    }
  }
