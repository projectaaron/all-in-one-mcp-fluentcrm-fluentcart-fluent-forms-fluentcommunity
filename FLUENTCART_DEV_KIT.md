# FluentCart Dev Kit — drop-in starter for a new FluentCart plugin

One file to copy into a new repo. It contains, in order:

1. **Setup steps** — 5 minutes to a repo with the complete per-endpoint API docs.
2. **Hard-won integration gotchas** — everything that bit us building the
   ShipFusion integration, so you don't rediscover it.
3. **Reusable code** — an internal REST client, admin-menu registration that
   survives FluentHub, and secrets encryption.
4. **The API docs generator + weekly auto-refresh workflow** (copy out to files).
5. **The full FluentCart development reference** (architecture, DB schema,
   models, 315+ hooks, REST API, boilerplate) — appended verbatim at the end.

---

## 1. Setup — get the complete API docs into this repo

The reference at the bottom of this file covers everything one-line-per-endpoint.
For **full request/response schemas on all ~380 endpoints**, generate them:

1. Create `scripts/gen-api-docs.mjs` from section 4 below.
2. Run `node scripts/gen-api-docs.mjs` (Node 20+, needs internet to
   dev.fluentcart.com). It auto-discovers every endpoint from the official docs
   sidebar and writes `docs/api/` — one markdown file per resource group with
   full request params, body schemas, and examples. It also writes
   `scripts/api-operations.txt` (the discovered operation list) so future runs
   can flag added/removed endpoints.
3. (Optional) Create `.github/workflows/refresh-api-docs.yml` from section 5 so
   CI re-runs the generator weekly and opens a PR only when FluentCart's API
   actually changed. Set `NOTIFY_USER` to your GitHub username.
4. If this plugin will auto-update from a private repo via zipball, add a
   `.gitattributes` marking `docs/`, `scripts/`, and repo meta `export-ignore`
   so only runtime code ships to sites.

---

## 2. Integration gotchas (learned the hard way)

These are verified against FluentCart in production. Do not skip.

### Hooks fire a single context array — not positional args
```php
do_action('fluent_cart/order_status_changed', [
    'order'      => $order,          // the model
    'old_status' => 'processing',
    'new_status' => 'completed',
]);
```
Normalize defensively in every handler — some hooks/versions pass positional
args instead:
```php
public function onStatusChanged($order, string $old = '', string $new = ''): void
{
    if (is_array($order) && isset($order['order'])) {
        $new   = $order['new_status'] ?? $new;
        $old   = $order['old_status'] ?? $old;
        $order = $order['order'];
    }
    // ...
}
...
### Status updates are fragile
- FluentCart treats `completed` as terminal — the REST API can return **200
  without applying** the change. Always verify against the DB after a REST
  status write, and fall back to a direct `$wpdb->update()` on `fct_orders`.
- `PUT /orders/{id}/statuses` needs an `action` discriminator
  (`change_order_status` / `change_shipping_status`) with
  `statuses.order_status` / `statuses.shipping_status` — one request per
  status type. `{statuses: {status: ...}}` silently fails.
- The REST path fires `order_status_changed` internally but NOT the
  `shipping_status_changed*` hooks — fire those manually (context array!) if
  you need shipping emails.
- Use direct DB writes for status changes made *from inside hooks/cron* to
  avoid recursive loops through the REST API.
- `Order::find()` can return stale cached data — use `Order::query()->find()`.

### Activity log (`fct_activity`) schema
- `module_type` = full model class path (`FluentCart\App\Models\Order`), not `'order'`
- `module_name` = `'order'`, `created_by` = a **string** (e.g. your plugin name)
- Prefer the `FluentCart\App\Models\Activity` Eloquent model; fall back to a
  direct insert with `created_at`/`updated_at`.

### Tables you must NOT write directly
- `fct_order_meta` does **not** use `key`/`value` column names — don't touch it.
- `fct_orders` has **no** `note` column — no "order note" writes.

### SKU / variation matching
`fct_product_variations` has BOTH `sku` and `variation_identifier` columns —
query/index both. Verify the `sku` column exists first (`SHOW COLUMNS`); older
installs may not have it. The REST route for variants is
`GET /products?with[]=variants` (there is no standalone `/products/variants`).

### Inventory updates
`PUT /products/{product}/update-inventory/{variation}` requires **both**
`total_stock` and `available` — sending only one gets rejected.

### List filtering
Use the array params `order_statuses[]` / `payment_statuses[]` on
`GET /orders`. The singular `status`/`payment_status` params are ignored — you
would silently receive unpaid orders.

### dbDelta (custom tables)
NO `IF NOT EXISTS` in the CREATE TABLE (dbDelta mis-parses the table name and
silently never migrates again), two spaces after `PRIMARY KEY`, one column/key
per line, prefer `LONGTEXT` over `JSON` column type for portability.

### General WordPress traps that bit us
- `wp_unslash($_POST)` before storing credentials (slashes corrupt passwords).
- Anything read on *every* request should be an autoloaded option.
- Never log full API payloads by default — they carry customer PII. Gate
  verbose logging behind an option/filter and prune log files on a schedule.
- Catch `\Throwable` (not just your own exception class) at hook boundaries —
  an uncaught exception inside a payment hook 500s the gateway's IPN request.

---

## 3. Reusable code

### Internal REST client (no HTTP overhead, correct auth context)
```php
class FluentCartClient
{
    const REST_NAMESPACE = 'fluent-cart/v2';

    /** @var int|null Cached admin user ID (null = not looked up, 0 = none found). */
    private static $admin_user_id = null;

    public function get(string $endpoint, array $params = []): array  { return $this->dispatch('GET', $endpoint, $params); }
    public function post(string $endpoint, array $body = []): array   { return $this->dispatch('POST', $endpoint, $body); }
    public function put(string $endpoint, array $body = []): array    { return $this->dispatch('PUT', $endpoint, $body); }
    public function delete(string $endpoint): array                   { return $this->dispatch('DELETE', $endpoint); }

    private function dispatch(string $method, string $endpoint, array $data = []): array
    {
        $route   = '/' . self::REST_NAMESPACE . '/' . ltrim($endpoint, '/');
        $request = new \WP_REST_Request($method, $route);

        if (in_array($method, ['GET', 'DELETE'], true)) {
            $request->set_query_params($data);
        } else {
            // Set BOTH raw JSON body and parsed params so FluentCart can read
            // via get_json_params() or get_body_params().
            $request->set_header('Content-Type', 'application/json');
            $request->set_body(wp_json_encode($data));
            $request->set_body_params($data);
        }

        // Cron/webhooks run with no user — elevate to an admin for the call,
        // then restore. Cache the lookup per request.
        $original_user = $this->ensureAdminContext();
        try {
            $response = rest_do_request($request);
        } finally {
            if ($original_user !== false) {
    ...
    private function ensureAdminContext()
    {
        $current = get_current_user_id();
        if ($current && user_can($current, 'manage_options')) {
            return false;
        }
        if (self::$admin_user_id === null) {
            $admins = get_users(['role' => 'administrator', 'number' => 1, 'fields' => 'ID']);
            self::$admin_user_id = empty($admins) ? 0 : (int) $admins[0];
        }
...
### Register your plugin inside FluentCart's admin (FluentHub-safe)
```php
// 1. Card in FluentCart → Settings → Plugin Addons
add_filter('fluent_cart/module_settings/plugin_addons', function (array $addons) {
    $addons['my-plugin-slug'] = [
        'title'       => __('My Plugin', 'my-textdomain'),
        'description' => __('What it does. Open the My Plugin menu item to configure.', 'my-textdomain'),
        'logo'        => MY_PLUGIN_URL . 'assets/images/logo.svg',
        'plugin_slug' => dirname(plugin_basename(MY_PLUGIN_FILE)),
...
// NOTE: the card's "Configure" cog is hardcoded to FluentCart's own fluent-pdf
// addon — third parties can't attach to it. The nav item below is your
// configure path.

// 2. Item in FluentCart's own nav ("more" group) — survives FluentHub
add_filter('fluent_cart/global_admin_menu_more_items', function (array $items) {
    $items['my_plugin'] = [
        'label'      => __('My Plugin', 'my-textdomain'),
        'link'       => admin_url('admin.php?page=my-plugin-settings'),
        'permission' => ['is_super_admin'],
    ];
    return $items;
});

// 3. WP-admin sidebar: under FluentCart's menu when visible, PLUS inside
// FluentHub's combined-menu flyout when FluentHub is active. Register the
// FluentHub pass late (priority 9999) so its menu exists; detect it by known
// slugs ('fluent-hub'/'fluenthub'/'fluent_hub') or by scanning $menu for a
// slug/title containing fluent+hub, then add_submenu_page() under it.
// Standalone top-level page only when neither Fluent menu exists.
```

### Secrets at rest (AES-256-CBC keyed from WP salts)
```php
public static function encrypt(string $plain): string
{
    ...
    if (function_exists('openssl_encrypt')) {
        ...
        return 'enc:v2:' . base64_encode($iv . openssl_encrypt($plain, 'AES-256-CBC', $key, OPENSSL_RAW_DATA, $iv));
    }
    return 'enc:b64:' . base64_encode($plain); // no-OpenSSL fallback — decrypt must mirror this
}
```
Keep encrypt/decrypt in ONE class. Remember: rotating WordPress salts bricks
every stored secret — detect decrypt failures and show an admin notice asking
for re-entry instead of failing silently.

---

## 4. `scripts/gen-api-docs.mjs` — full per-endpoint docs generator

Copy everything inside the fence into `scripts/gen-api-docs.mjs`:
...
## 5. `.github/workflows/refresh-api-docs.yml` — weekly auto-refresh

Copy into `.github/workflows/refresh-api-docs.yml` and change `NOTIFY_USER`.
Remove the ShipFusion generator step (or keep the pattern for your own second
source). Base branch in the `gh pr create` call should match your default
branch:

````yaml
name: Refresh API docs
# Regenerates the FluentCart docs (docs/api/) and the ShipFusion reference
# (docs/shipfusion-api-reference.md) from their upstream sources on a schedule,
# and opens (or updates) a single PR whenever either API has changed. Runs on
# GitHub's runners, which have outbound internet to dev.fluentcart.com and
# jsapi.apiary.io.
on:
  schedule:
...
jobs:
  refresh:
    ...
    steps:
      ...
      - name: Regenerate FluentCart API docs (OpenAPI specs)
        run: node scripts/gen-api-docs.mjs

      - name: Regenerate ShipFusion API reference (Apiary)
        run: node scripts/gen-shipfusion-api-docs.mjs

      - name: Open or update a PR if the API changed
        env:
          GH_TOKEN: ${{ secrets.GITHUB_TOKEN }}
          # Who to notify (review request + assignee + @mention). GitHub emails
          # these by default per the recipient's notification settings.
          NOTIFY_USER: projectaaron
        run: |
          set -euo pipefail
          # git status --porcelain (not `git diff --quiet`) so newly CREATED
          # doc files — e.g. a brand-new API resource group — count as changes.
          if [ -z "$(git status --porcelain)" ]; then
            echo "No API changes — nothing to do."
            exit 0
          fi

          BRANCH="auto/api-docs-refresh"
          git config user.name  "github-actions[bot]"
          git config user.email "41898282+github-actions[bot]@users.noreply.github.com"
          git switch -c "$BRANCH"
          git add -A
          git commit -m "docs(api): scheduled FluentCart API docs refresh"
          git push -f -u origin "$BRANCH"

          # Summarize what changed for the PR body.
          SUMMARY=$(git show --stat --oneline HEAD | head -40)

          if [ -z "$(gh pr list --head "$BRANCH" --state open --json number --jq '.[].number')" ]; then
            gh pr create \
              --base claude/main \
              --head "$BRANCH" \
              --title "docs(api): scheduled FluentCart API docs refresh" \
              --assignee "$NOTIFY_USER" \
              --reviewer "$NOTIFY_USER" \
              --body "$(printf 'cc @%s — automated refresh via `scripts/gen-api-docs.mjs`; the upstream FluentCart API changed.\n\nReview the diff for **added / removed endpoints** and any schema changes, then merge.\n\nChanged files:\n```\n%s\n```\n\n🤖 Generated by the Refresh API docs workflow.' "$NOTIFY_USER" "$SUMMARY")" \
              || gh pr create --base claude/main --head "$BRANCH" \
                   --title "docs(api): scheduled FluentCart API docs refresh" \
                   --body "cc @${NOTIFY_USER} — automated API docs refresh (reviewer/assignee could not be set automatically)."
          else
            # PR already open — force-push above updated it; ping with a comment so
            # the new changes are noticed too.
            gh pr comment "$BRANCH" --body "@${NOTIFY_USER} refreshed with the latest upstream API changes." || true
...
## 6. How the docs generation works (method notes)
...
# THE FULL FLUENTCART DEVELOPMENT REFERENCE

Everything below is the complete generated reference (architecture, database
schema, models, hooks, REST API overview, boilerplate). Regenerate the
per-endpoint detail with the script above whenever FluentCart updates.
